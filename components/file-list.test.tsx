...
describe("FileList", () => {
  it("shows empty state when no textbooks", () => {
    render(<FileList textbooks={[]} selectedId={null} onSelect={vi.fn()} onMerge={vi.fn()} />);
    expect(screen.getByText("暂无教材")).toBeInTheDocument();
  });

  it("renders all textbooks", () => {
    render(
      <FileList textbooks={sampleTextbooks} selectedId={null} onSelect={vi.fn()} onMerge={vi.fn()} />,
    );
  ...
  it("highlights selected textbook", () => {
    const { container } = render(
      <FileList textbooks={sampleTextbooks} selectedId="tb1" onSelect={vi.fn()} onMerge={vi.fn()} />,
    );
  ...
  it("calls onSelect when textbook clicked", () => {
    ...
    render(
      <FileList textbooks={sampleTextbooks} selectedId={null} onSelect={onSelect} onMerge={vi.fn()} />,
    );
